...
public class Closeconfig_to_NoReward extends BaseCase {
	...
	public void Closeconfig_to_NoRewardtest1() {
		...
		JSONObject failtoencash = Reward_New.performEncash(g_user,
				encashExpected);
	...
	public void Closeconfig_to_NoRewardtest4() {
		...
		assertEquals("共享wifi领现金活动开启",
				checkconfig_off.getJSONArray("missionConfigList")
						.getJSONObject(1).getInt("missionStatus"), 0);

		// 拉取共享wifi得时长配置，确认已关闭
		JSONObject checkconfig_off1 = config_New.getconfig(g_user);
		assertEquals("共享wifi领时长活动开启",
				checkconfig_off1.getInt("win_time_switch"), 1);
	}
