...
public class RewardTestcase_New extends BaseCase {
	...
	public void rewardTestcase_New1() {
		...
		JSONObject encashObject = Reward_New.performEncash(g_user,
				encashExpected);
		// 接口验证返回值为0是成功
		...
		// 验证获取最新提现详情
		JSONObject LatestEncashDetailObject = Reward_New
				.getLatestEncashDetail(g_user);
		assertEquals("提现用户的userid不相同",
				LatestEncashDetailObject.getString("userId"), g_user.getUid());
	...
	public void rewardTestcase_New3() {
		...
		User testuser = new User("oGsKms0G_IG6cvXk0sKje9sP89XU", "青青的小树",
				"weixin");
		// 验证查询用户提现记录
